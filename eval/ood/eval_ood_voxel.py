"""Shared OOD voxel evaluation logic.

Loads a pre-built test-set .npy (no train/val/test split), slices by index
range for fine_grid (0:1000) or two_inclusions (1000:2000), and evaluates
trained models from grid_search_full_2.
"""
import argparse
import json
...
def build_ood_test_loader(
    ...
    sigma = data["Y"][start:end].astype(np.float32, copy=False)
    sigma = sigma[:, None, ...]  # add channel dim
    ...
    y = torch.from_numpy(sigma).to(device)
...
def save_sample(
    ...
    if subset_label:
        subtitle = f"[{subset_label}]  " + subtitle
...
def eval_run(
    ...
    print(f"  evaluating {run_name} on {subset} ...")
...
def run_ood_eval(subset: str, default_output_root: str) -> None:
    ...
    parser.add_argument("--mat", type=str, default=str(REPO_ROOT / "data" / "sample1.mat"),
                        help="Path to sample*.mat for electrode overlay")
    ...
    for noise_level in noise_levels:
        ...
        if run_summaries:
            ...
            noise_str = f" at noise {noise_level}" if noise_level is not None else ""
            print(f"  Best (L2){noise_str}: {sorted_l2[0]['run']}  (mean_bs_rel_l2={sorted_l2[0]['mean_bs_rel_l2']:.4f})")
            print(f"  Best (L1){noise_str}: {sorted_l1[0]['run']}  (mean_bs_rel_l1={sorted_l1[0]['mean_bs_rel_l1']:.4f})")
            summary_l2 = {
                "subset": subset,
                "noise_level": noise_level,
            ...
            with (level_dir / "ood_summary_l2.json").open("w", encoding="utf-8") as f:
                json.dump(summary_l2, f, indent=2)
            with (level_dir / "ood_summary_l1.json").open("w", encoding="utf-8") as f:
