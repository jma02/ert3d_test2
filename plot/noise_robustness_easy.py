"""Plot noise robustness curves and generate LaTeX tables for the easy dataset.

Usage:
    PYTHONPATH=. uv run python plot/noise_robustness_easy.py
"""
import json
...
SWEEP_PATH = REPO_ROOT / "eval_outputs" / "grid_search_no_encoder-noise" / "noise_sweep.json"
...
def _find_top_models(sweep: dict) -> dict[str, str]:
    ...
    labels = {}
    for m in ordered:
        # extract key params from run name
        parts = m.split("_")
...
def plot_easy(sweep: dict, top_models: dict[str, str]) -> None:
    fig, axes = plt.subplots(1, 2, figsize=(14, 5), sharey=False)
    fig.suptitle("Noise Robustness — Easy Dataset", fontsize=14, fontweight="bold")
    ...
    out = OUTPUT_DIR / "noise_robustness_easy.png"
...
def latex_tables(sweep: dict, top_models: dict[str, str]) -> str:
    lines: list[str] = []
    ...
    lines.append(r"    \caption{Noise robustness, easy dataset. Best value per noise level in \textbf{bold}.}")
    lines.append(r"    \label{fig:noise-tables-easy}")
...
def main() -> None:
    ...
    plot_easy(sweep, top_models)

    tex = latex_tables(sweep, top_models)
    out = OUTPUT_DIR / "noise_robustness_easy_tables.tex"
    out.write_text(tex)
    print(f"Saved {out}")
    print(tex)
